...
export async function POST(req: NextRequest, { params }: { params: { managerId: string } }) {
  ...
  const manager = await prisma.propertyManager.findUnique({ where: { id: params.managerId } });
  if (!manager) return NextResponse.json({ error: "Manager not found" }, { status: 404 });

  const review = await prisma.$transaction(async (tx) => {
    const r = await tx.managerReview.create({
      data: { managerId: params.managerId, reviewerId, rating, comment },
    });
    const allReviews = await tx.managerReview.findMany({ where: { managerId: params.managerId } });
